...
export async function cancelLendOffer(offerId: number, lenderAgentId: string) {
  ...
  const lender = await getAgentById(lenderAgentId);
  const currentRep = await blockchain.getAgentRep(lender.wallet_address);
  const newScore = Math.max(0, currentRep.score - 1);

  await blockchain.setReputation(lender.wallet_address, newScore, "offer_cancel_penalty");
  ...
  for (const agent of dormant) {
    try {
      ...
      if (newScoreInt < agent.reputation_score) {
        await blockchain.setReputation(agent.wallet_address, newScoreInt, "inactivity_decay");
