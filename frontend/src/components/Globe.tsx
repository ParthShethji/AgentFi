...
function AgentDot({ position, index }: { position: THREE.Vector3; index: number }) {
  ...
  return (
    <mesh ref={meshRef} position={position}>
      ...
      <pointLight color="#10B981" intensity={0.3} distance={0.4} />
    </mesh>
  );
}
...
function CommunicationLines({ dots }: { dots: THREE.Vector3[] }) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < MAX_LINES; i++) {
      ...
      const mat = new THREE.LineBasicMaterial({ color: '#10B981', transparent: true, opacity: 0 });
...
function SceneLights() {
  return (
    <>
      ...
      <pointLight position={[-3, -2, 1]} intensity={0.3} color="#10B981" />
    </>
  );
}
